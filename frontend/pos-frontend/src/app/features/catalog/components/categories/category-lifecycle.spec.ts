...
describe('Category lifecycle', () => {
  const item = { id: 1, name: 'Categoría histórica', isActive: true };

  beforeEach(() => {
    TestBed.configureTestingModule({
      imports: [CategoriesTable, CategoryDialog],
  ...
  it('renders Desactivar for active records and Activar for inactive records, without delete', async () => {
    vi.spyOn(TestBed.inject(CategoryService), 'getAll').mockReturnValue(of([item, { ...item, id: 2, isActive: false }]));
    const fixture = TestBed.createComponent(CategoriesTable);
    ...
    expect(TestBed.inject(CategoryService)).not.toHaveProperty('delete');
  });
  ...
  it('confirms lifecycle, posts explicit endpoints and refreshes the table', () => {
    ...
    for (const active of [true, false]) {
      ...
      const request = requests.expectOne((req) => req.url.endsWith(`/api/Categories/1/${active ? 'deactivate' : 'activate'}`));
  ...
  it('keeps activation out of the edit form and emitted HTTP payload', () => {
    const dialog = TestBed.runInInjectionContext(() => new CategoryDialog());
    dialog.category = item;

    dialog.form.patchValue({ name: item.name });
    const events: CategoryDialogSubmit[] = [];
    ...
    const payload = { ...item };
    TestBed.inject(CategoryService).update(item.id, payload).subscribe();
  ...
  it('reports a failed lifecycle change and preserves the loaded records', () => {
    ...
    TestBed.inject(HttpTestingController).expectOne((req) => req.method === 'POST')
      .flush({ error: 'CATEGORY_HAS_ACTIVE_PRODUCTS' }, { status: 409, statusText: 'Conflict' });
